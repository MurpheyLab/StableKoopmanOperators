...
class PusherSlider(object):
    def __init__(self, render=False, time_step=1./100., frame_skip=10, seed=None):
        if seed is not None:
            np.random.seed(seed)
        ...
        if render:
            ...
            bullet_client.resetDebugVisualizerCamera(1, 90, -40, [0.,0.,0.])
        else:
    ...
    def state(self):
        ...
        return np.array([
            slider_pos[0], slider_pos[1], mat2euler(slider_mat)[-1],
            pnt_pos[0], self._vb[0], self._vb[1]
        ])

    def reset(self):
        self.slider_pos[0] = np.random.normal(0.3, 0.01) # np.random.uniform(0.27,0.32)
        self.slider_pos[1] = np.random.normal(0.01, 0.01) # np.random.uniform(0., 0.01)
        self.ee_pos = self.slider_pos + self.ee_block_offset
    ...
    def step(self, ab): # pass in velocity in body frame
        ...
        return self.state.copy(), ab
        # return np.array([
        #     slider_pos[0], slider_pos[1], np.array([mat2euler(slider_mat)[-1]]),
        #     pnt_pos[0], vb[0], vb[1]
        # ])
